...
def catalog_json():
    ...
    all_categories = (session.query(Categories).options(
        joinedload(Categories.item)).all())
    return jsonify(Catalog=[dict(c.serialize, all_items=[i.serialize
                                                         for i in c.item])
...
def add_category():
    ...
    if request.method == 'POST':
        ...
        category_name_in_db = (session.query(Categories.title)
